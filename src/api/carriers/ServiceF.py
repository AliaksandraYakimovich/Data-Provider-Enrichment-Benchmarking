...
class ServiceF(BaseService):
    def __init__(self):
        super().__init__("SERVICE_F")
        self.api_key = str(Config.SERVICE_F_API_KEY)
        self.api_url_base = (
            "https://api.service-f.provider.com/gateway/v1/tracking-events"
        )

    def fetch_data(self, entity_id: str, reference_id: str = "") -> Dict[str, Any]:
        if not self.api_key:
            return {"error": "SERVICE_F API Key Missing"}
        if not reference_id or str(reference_id).strip().lower() in ["", "nan", "none"]:
            return {"error": "Missing Reference ID"}

        # Query parameters genericized
        url = f"{self.api_url_base}?providerReference={reference_id}&entityReference={entity_id}"
    ...
    def parse_response(self, raw_data: Any, target_location: str = "") -> Dict[str, Any]:
        ...
        for event in location_events:
            ...
            if t_mode == "VESSEL": # Kept for payload integrity
                # take transportEventTypeCode, but if missing then try equipmentEventTypeCode or shipmentEventTypeCode
                e_code = str(
        ...
        if primary_events:
            # devide events into ACTUAL and ESTIMATED/PLANNED
            act_events = [
                e
            ...
            if act_events:
                # sort actual events by date and take the earliest one
                act_events.sort(key=lambda x: x.get("eventDateTime", ""), reverse=False)
                best_event = act_events[0]
            else:
                # no factual events, take the latest estimated event
